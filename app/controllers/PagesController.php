	...
	*/
	...
	public function search() {
		...
		if ($needle != "") {
			...
			}elseif ((DateTime::createFromFormat('Y-m-d', $needle) !== FALSE || 
				DateTime::createFromFormat('Y-m-d H:i:s', $needle) !== FALSE )&& !empty($type)) {
	...
	public function advancedSearch() {
        ...
        if( count($fields) == 0 ) {
            if(Session::has('last_search'))
                return View::make('/advanced-search', Session::get('last_search'));
            else { // busca vazia
		...
		Session::put('last_search', ['tags' => $tags, 'photos' => $photos,
